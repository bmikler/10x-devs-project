...
4. Open the report and see, for that category, how much remains for the current
   year, alongside a spend metric whose shape follows the category's type —
   an average monthly spend for recurring categories, and a single cumulative
   spent value for irregular categories.
...
- FR-003: User can create a category with a name, a type (recurring monthly or
  ...
  > identified in Phase 1 as one of the three core product insights. Dropping
  > it would
  > gut the product.
